...
def message_handler(c, event, *args):
    ...
    # ----------------------------------------------------
    #  ✅ ДОБАВЛЕНИЕ ЛОГИКИ КОМАНДЫ /ACTIVATE
    # ----------------------------------------------------
    ...
    if text_lower.startswith("/activate"):
        ...
        if len(parts) == 2:
            key = parts[1].upper()
            logger.info(f"{LOGGER_PREFIX} Запущена активация ключа {key} для {username} ({user_id})")
            # Теперь вызываем функцию, которая уже работает с GitHub!
            activate_key(c, user_id, username, key)
            return
        else:
            # Отправка ошибки, если ключ не указан
            send_message(c, user_id, "🚫 Укажите ключ активации. Пример: /activate КЛЮЧ-АГЕНТА")
            return
    
    # ----------------------------------------------------
    #  КОНЕЦ ЛОГИКИ КОМАНДЫ
    # ----------------------------------------------------
